...
final class ShopController
{
    ...
    public function checkout(Request $request):Response{if(!Auth::user())return Response::redirect('/login?next='.rawurlencode('/checkout'));return Response::html(View::render('shop/checkout',['title'=>'Checkout — Genezenz Pharmacy','user'=>Auth::user(),'prescriptions'=>$this->prescriptions->availableForOrder((int)Auth::user()['id'])]));}
    public function placeOrder(Request $request):Response
    {
        ...
        try{$orderNo=$this->orders->create((int)$user['id'],is_array($input['items']??null)?$input['items']:[],$customer,isset($input['prescription_id']) && $input['prescription_id']!==''?(int)$input['prescription_id']:null);return Response::json(['message'=>'Order placed.','orderNo'=>$orderNo,'redirect'=>'/order/'.$orderNo],201);}catch(\PDOException$e){error_log('Order database error: '.$e->getMessage());return Response::json(['message'=>'The order service is temporarily unavailable. Please try again.'],503);}catch(\Throwable$e){return Response::json(['message'=>$e->getMessage()],422);}
    }
