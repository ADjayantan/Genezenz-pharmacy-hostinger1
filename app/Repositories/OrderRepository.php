...
final class OrderRepository
{
    ...
    public function create(int $userId, array $lines, array $customer): string
    {
    ...
    public function updateStatus(string $orderNo, string $status, string $actor, array $tracking=[]): void
    {
        ...
        $this->database->beginTransaction();
        try {
            $s=$this->database->prepare('SELECT id,status,stock_restored_at FROM orders WHERE order_no=:no FOR UPDATE'); $s->execute(['no'=>$orderNo]); $order=$s->fetch();
